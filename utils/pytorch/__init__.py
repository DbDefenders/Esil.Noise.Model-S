...
def initialization(training_dataloader, model):
    ...
    if TrainsParams["optimizer"] == "adam":
        optimizer = torch.optim.AdamW(
            ...
            lr=TrainsParams["learning_rate"],
            weight_decay=TrainsParams["weight_decay"],
        )
    else:
        try:
...
def train_an_epoch(
    ...
    scaler,
    loss_func,
    tag_wandb: bool = False,
    *args,
):
    trainloss = 0
    model.train()  # 初始化训练损失为0，并设置模型为训练模式
    count = 0  # 初始化计数器

    for idx, (features, label) in enumerate(
        tqdm(training_dataloader, leave=False, desc="[train]")
    ):  # 遍历数据加载器，使用tqdm显示进度条
        ...
        ):  # 决定是否启用自动混合精度训练，并设置数据类型为torch.bfloat16
    ...
    return (
        ...
        scheduler,
        trainloss,
    )  # 返回更新后的模型、优化器、梯度缩放器、学习率调度器和平均训练损失


# endregion
# region test
def test_an_epoch(
    model, testing_dataloader, loss_func, LABELS: list, tag_wandb: bool = False, *args
):
    ...
    with torch.no_grad():  # 在此上下文中，所有计算都不会跟踪梯度
        for idx, (features, label) in enumerate(
            tqdm(testing_dataloader, leave=False, desc="[valid]")
        ):

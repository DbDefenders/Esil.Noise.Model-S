...
class KAN_Convolutional_Layer(torch.nn.Module):
    def __init__(
        ...
        Args:
            ...
            dilation (tuple): 卷积膨胀
            grid_size (int): 网格的大小
            spline_order (int): 插值曲线的阶数
            scale_noise (float): 噪声的缩放因子
            scale_base (float): 基函数的缩放因子
            scale_spline (float): 插值曲线的缩放因子
            base_activation (torch.nn.Module): 基函数的激活函数
            grid_eps (float): 网格的精度

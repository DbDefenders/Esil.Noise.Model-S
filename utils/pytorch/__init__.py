...
def initialization(training_dataloader):
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        ...
        pct_start=0.0, steps_per_epoch=len(training_dataloader),
...
def test_an_epoch(model, training_dataloader, loss_func, LABELS:list, *args):
    ...
    # 计算auc,prec,rec,acc
    auc = auroc(preds=preds_tensor, task='multiclass', target=targets_tensor, num_classes=len(LABELS),
                             average="macro")  # 计算AUC

    prec = precision(preds=preds_tensor, task='multiclass', target=targets_tensor, num_classes=len(LABELS),
                             average="macro")  # 计算精确度
    rec = recall(preds=preds_tensor, task='multiclass', target=targets_tensor, num_classes=len(LABELS),
                             average="macro")  # 计算召回率
    
    acc = accuracy(preds=preds_tensor, task='multiclass', target=targets_tensor, num_classes=len(LABELS),
                             average="macro")  # 计算准确率

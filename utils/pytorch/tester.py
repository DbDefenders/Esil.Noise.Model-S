...
class Tester(ModelManager):
    def __init__(
        ...
        self.f1_score_func = self.get_metrics_func(
            f1_score_func, average="macro"
        )
        self.f1_score_micro_func = self.get_metrics_func(
            f1_score_func, average="micro"
        )
        # endregion
    ...
    def test_an_epoch(self, get_file_path: callable = None)->tuple[TestMetrics, wandb.Table]:
        ...
        columns=["index",'Filename',"label","y_pred"] # idx, filepath, label, y_pred

        bad_cases = wandb.Table(columns=columns)
        ...
        with torch.no_grad():  # 在此上下文中，所有计算都不会跟踪梯度
            for count, (features, labels, idxes) in enumerate(
                ...
                for i, result in enumerate(results):
                    label = labels[i].item()
                    y_pred = outputs.argmax(1)[i].cpu().numpy()
                    idx = idxes[i].item()
                    ...
                    if not result:
                        bad_cases.add_data(idx, filepath, label, y_pred)
        ...
        # endregion
        return metrics_, bad_cases  # 返回测试指标
